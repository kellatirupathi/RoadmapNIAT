...
// Removed the 'critical-points' entry
...
const InternshipsTracker = () => {
    ...
    const renderActiveSheet = () => {
        ...
        const cardHeader = (title, showAddButton = true, showUploadButton = true, onAddClick = null, onUploadClick = null) => (
            <Card.Header as="h6" className="bg-light d-flex justify-content-between align-items-center">
                {title}
        ...
        if (activeSheet === 'internship-master') { 
             return (
                <div>
                     <Card className="shadow-sm">
                        {cardHeader(config.name, true, true, () => { setEditingInternship(null); setShowEditModal(true); }, () => handleOpenUploadModal(activeSheet))}
        ...
        if (activeSheet === 'tech-stack-roadmaps') { 
            return (
                <div>
                     <Card className="shadow-sm">
                        {cardHeader(config.name, true, true, () => { setEditingTechStackRoadmap(null); setShowTechStackRoadmapModal(true); }, () => handleOpenUploadModal(activeSheet))}
                        <Card.Body className="p-0">
        ...
        if (['student-wise-progress'].includes(activeSheet)) {
            return (
                <>
                    {user.role === 'admin' && renderAddFormForSheet(activeSheet)}
                    <Card className="shadow-sm">
                        <Card.Header as="h6" className="bg-light">{name} Data</Card.Header>
                        <Card.Body className="p-0"> 
                            <div className="table-container-scroll">
                                <EditableTable columns={columns} data={paginatedData} onSave={user.role === 'admin' ? handleSaveRow : undefined} onDelete={user.role === 'admin' ? openDeleteRowModal : undefined} isLoading={actionLoading || loading} allowAdd={false} activeSheet={activeSheet} onAdd={handleSaveFromModal(activeSheet)} />
                            </div>
                        </Card.Body>
                        <Card.Footer className="bg-light"><PaginationControls totalRows={sheetData.length} /></Card.Footer>
                    </Card>
                            ...
                            <div className="table-responsive" style={{ maxHeight: '40vh', overflowY: 'auto' }}><Table striped bordered size="sm">
                                ...
                                <tbody>
                                    {csvData.map((row, rowIndex) => (
                                        <tr key={rowIndex}>
                                            {csvHeaders.map((header, colIndex) => <td key={colIndex} title={row[header]}>{row[header]}</td>)}
                                        </tr>
                                    ))}
                                </tbody>
                            </Table></div>
