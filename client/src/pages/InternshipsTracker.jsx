...
// --- NEW COMPONENT: For the 3-dot actions menu ---
            ...
            <tr key={`${company._id}-mapping-${index}`} className={!isFirstMapping ? "bg-light" : ""}>
                {isFirstMapping && (
                    <>
                        ...
                        <td rowSpan={company.mappings?.length || 1} style={{ minWidth: '200px', whiteSpace: 'normal', wordBreak: 'break-word' }}>
                            {(company.companyStatus === 'Inactive' || company.companyStatus === 'Hold') ? company.reasonInactive : ''}
                        </td>
                        <td rowSpan={company.mappings?.length || 1}>{company.studentMappingMethod}</td>
                        <td rowSpan={company.mappings?.length || 1}>{company.studentMappingCounts}</td>
                ...
                {user.role === 'admin' && isFirstMapping && (
                    <td rowSpan={company.mappings?.length || 1} className="text-center">
                        <Dropdown>
                            <Dropdown.Toggle as={ActionMenuToggle} id={`actions-dropdown-${company._id}`} />
                            <Dropdown.Menu align="end">
                                <Dropdown.Item onClick={() => handleOpenEditModal(company, 'internship-master')}>
                                    <i className="fas fa-edit me-2 text-primary"></i> Edit
                                </Dropdown.Item>
                                <Dropdown.Item onClick={() => openDeleteRowModal(company._id)} className="text-danger">
                               ...
                               <Table striped bordered hover size="sm" className="align-middle">
                                   <thead className="table-light">
                                       <tr>
                                           ...
                                           <th>Offers</th>
                                           <th>Status</th>
                                           <th style={{ minWidth: '200px' }}>Reason (Inactive)</th>
                                           <th>Mapping Method</th>
                                           <th>Mapping Counts</th>
                                           <th>Mapping Offers</th>
                                           <th>Tech Stacks & Progress</th>
