...
//   // UPDATED: Optimized handlers for silent state updates
//   const handleRoadmapCreated = (newRoadmap) => {
//     setRoadmaps(prev => [newRoadmap, ...prev]);
//     setSuccess('New roadmap created successfully!');
...
//         if (!newRoadmap.isConsolidated) { 
//           newRoadmap.techStacks = updateLogic(newRoadmap.techStacks); 
//         } else { 
//           newRoadmap.roles = (newRoadmap.roles || []).map(role => ({ 
//             ...role, 
//             techStacks: updateLogic(role.techStacks) 
//           })); 
//         }
...
//   // UPDATED: Silent update of roadmap in state
//   const handleSaveChanges = async (e) => {
//     e.preventDefault();
//     if (!editingRoadmap) return;
...
//       const payload = { 
//         ...editingRoadmap, 
//         filename: editingRoadmap.filename.endsWith('.html') 
//           ? editingRoadmap.filename 
//           : `${editingRoadmap.filename}.html` 
//       };
      
//       const response = await roadmapService.updateRoadmap(editingRoadmap._id, payload);
//       const updatedRoadmap = response.data;
      
//       // Update the roadmap in state without fetching all roadmaps again
...
//   // UPDATED: Silent removal from state
//   const handleDelete = async () => {
//     if (!roadmapToDelete) return;
//     setActionLoading(true);
...
//       // Remove deleted roadmap from state without fetching all roadmaps again
//       setRoadmaps(prev => prev.filter(r => r._id !== roadmapToDelete._id));
...
//     return searchMatch && companyMatch && roleMatch && crmMatch && statusMatch;
//   }), [roadmaps, searchTerm, crmUserMap, companyFilter, roleFilter, crmFilter, statusFilter]);
...
//   const handleExportCSV = () => {
//     if (filteredRoadmaps.length === 0) {
//       setError("No data available to export based on the current filters.");
//       return;
//     }
...
//                 onClick={stat.title === 'Unique CRMs Assigned' ? () => setShowCrmDetailsModal(true) : undefined} 
//                 style={stat.title === 'Unique CRMs Assigned' ? { cursor: 'pointer' } : {}}
//               >
//                   <Card.Body className="d-flex align-items-center">
//                       <div className={`bg-${stat.color} bg-opacity-10 rounded-circle p-3 me-3`}>
//                         <i className={`fas ${stat.icon} fa-lg text-${stat.color}`}></i>
//                       </div>
//                       <div>
//                           <div className="fs-4 fw-bold">{stat.value}</div>
//                           <div className="text-muted small">{stat.title}</div>
...
//                     <Form.Control 
//                       type="text" 
//                       placeholder="Search Company, Role, Tech Stacks..." 
//                       value={searchTerm} 
...
//                     {loading ? (
//                       <tr>
//                         <td colSpan="8" className="text-center p-5"><Spinner /></td>
//                       </tr>
//                     ) : paginatedData.length > 0 ? (
//                         paginatedData.map(roadmap => {
//                             const { status, percentage } = calculateRoadmapStatus(roadmap);
//                             const techStackNames = [...new Set(
//                               (roadmap.isConsolidated 
//                                 ? (roadmap.roles || []).flatMap(r => r.techStacks || []) 
//                                 : (roadmap.techStacks || [])
//                               ).map(ts => ts?.name).filter(Boolean)
//                             )];
//                             const roleNames = roadmap.isConsolidated 
//                               ? (roadmap.roles || []).map(r => r.title) 
//                               : [];
...
//                                       <Button 
//                                         variant="link" 
//                                         className="p-0 company-name-link" 
...
//                                     <td>
//                                       {roadmap.isConsolidated 
//                                         ? <Badge bg="primary">Consolidated</Badge> 
//                                         : roadmap.role
//                                       }
//                                     </td>
//                                     <td><OverflowDropdownDisplay items={roleNames} label="Roles" /></td>
//                                     <td><OverflowDropdownDisplay items={techStackNames} label="Stacks"/></td>
//                                     <td>
//                                       {crmUsers.find(c => c.username === roadmap.crmAffiliation)?.displayName 
...
//                                             <i className="fas fa-info-circle fa-fw me-2 text-info"></i>View Details
//                                           </Dropdown.Item>
//                                           <Dropdown.Item 
...
//                                           >
//                                             <i className="fas fa-external-link-alt fa-fw me-2 text-success"></i>Open Roadmap
//                                           </Dropdown.Item>
//                                           <Dropdown.Item onClick={() => handleEditClick(roadmap)}>
//                                             <i className="fas fa-edit fa-fw me-2 text-primary"></i>Edit/Re-publish
//                                           </Dropdown.Item>
//                                           {user.role === 'admin' && (
//                                             <>
//                                               <Dropdown.Divider />
...
//                                               >
//                                                 <i className="fas fa-trash-alt fa-fw me-2"></i>Delete Roadmap
//                                               </Dropdown.Item>
//                                             </>
//                                           )}
//                                         </Dropdown.Menu>
...
//                           No roadmaps found for the selected filters.
//                         </td>
//                       </tr>
//                     )}
//                 </tbody>
...
//       {/* UPDATED: Pass the handler for silent updates */}
//       <CreateRoadmapModal 
...
//             <Modal.Header closeButton>
//               <Modal.Title>Edit Roadmap: {editingRoadmap.companyName}</Modal.Title>
//             </Modal.Header>
//             <Modal.Body style={{ maxHeight: '75vh', overflowY: 'auto' }}>
//               <Tab.Container activeKey={activeEditTab} onSelect={(k) => setActiveEditTab(k)}>
...
//                             type="text" 
//                             name="companyName" 
//                             value={editingRoadmap.companyName || ''} 
...
//                             type="text" 
//                             name="filename" 
//                             value={editingRoadmap.filename ? editingRoadmap.filename.replace('.html', '') : ''} 
//                             onChange={handleEditModalInputChange} 
//                             required
//                           />
//                         </Form.Group>
...
//                           <Form.Select 
//                             name="crmAffiliation" 
//                             value={editingRoadmap.crmAffiliation || ''} 
//                             onChange={handleEditModalInputChange}
//                           >
...
//                       <strong>Note:</strong> Editing the core structure (e.g., adding/removing roles, changing tech stacks) requires creating a new roadmap. This section is for metadata and content edits only.
//                     </Alert>
//                   </Tab.Pane>
//                   <Tab.Pane eventKey="content">
//                     <h5 className="mb-3">Edit Content & Progress</h5>
//                     {success && <Alert variant="success" className="py-2 small">{success}</Alert>}
//                     {((editingRoadmap.isConsolidated 
//                       ? (editingRoadmap.roles || []).flatMap(r => r.techStacks || []) 
//                       : (editingRoadmap.techStacks || [])) || []
//                     ).map(tsCopy => tsCopy ? (
//                       <div key={tsCopy._id} className="mb-4">
//                         <TechStackTable 
//                           techStackData={tsCopy} 
//                           isEditable={true} 
//                           onUpdate={handleEmbeddedTechStackUpdate} 
//                           isEmbedded={true} 
//                         />
//                       </div>
//                     ) : null)}
...
//                 {actionLoading ? <Spinner size="sm"/> : "Save & Re-publish"}
//               </Button>
//             </Modal.Footer>
//           </Form>
//         </Modal>
...
//                   </h6> 
//                   <p className="mb-2">
//                     <strong>Name:</strong> {selectedRoadmapForDetails.companyName}
//                   </p> 
//                   <p className="mb-2">
//                     <strong>Filename:</strong> 
//                     <span className="text-muted">{selectedRoadmapForDetails.filename}</span>
//                   </p> 
//                   <p className="mb-0">
//                     <strong>Type:</strong> 
//                     <Badge bg={selectedRoadmapForDetails.isConsolidated ? 'primary' : 'info'}>
//                       {selectedRoadmapForDetails.isConsolidated ? 'Consolidated' : 'Single Role'}
//                     </Badge>
//                   </p> 
//                 </div>
//               </Col>
...
//                   </p> 
//                   <p className="mb-2">
//                     <strong>Status:</strong> <Badge bg="success">Published</Badge>
//                   </p> 
//                   <p className="mb-0">
//                     <strong>Assigned CRM:</strong> 
//                     <Badge pill bg="secondary" className="fw-normal">
//                       {crmUsers.find(c => c.username === selectedRoadmapForDetails.crmAffiliation)?.displayName 
...
//             </Row>
//             <div className="bg-white border rounded">
//               <h6 className="fw-bold p-3 border-bottom mb-0">
//                 <i className="fas fa-users-cog me-2 text-muted"></i>
...
//                     (selectedRoadmapForDetails.roles || []).map((role, i) => (
//                       <tr key={i}>
//                         <td>
//                           <i className="fas fa-user-tie me-2 text-secondary"></i>{role.title}
//                         </td>
//                         <td>
//                           {(role.techStacks || []).map(ts => (
//                             <Badge key={ts._id} bg="secondary" className="me-1 mb-1 fw-normal">
//                               {ts.name}
//                             </Badge>
//                           ))}
//                         </td>
...
//                         {selectedRoadmapForDetails.role}
//                       </td>
//                       <td>
//                         {(selectedRoadmapForDetails.techStacks || []).map(ts => (
//                           <Badge key={ts._id} bg="secondary" className="me-1 mb-1 fw-normal">
//                             {ts.name}
//                           </Badge>
//                         ))}
...
//                 <tr>
//                   <th>CRM Name</th>
//                   <th className="text-center">Roadmaps Assigned</th>
//                 </tr>
//               </thead>
//               <tbody>
//                 {crmStats.map(stat => (
...
//         </Modal.Header>
//         <Modal.Body>
//           Are you sure you want to delete the roadmap for <strong>{roadmapToDelete?.companyName}</strong>?
//         </Modal.Body>
//         <Modal.Footer>
...
//         /* Always open dropdown downward */
//         .dropdown-menu.actions-dropdown-menu.show {
...
//         /* Make sure each actions cell has proper positioning */
//         td.text-center {
//           position: static;
//         }
        
//         /* Ensure entire table row has positioning context */
//         .table tr {
//           position: relative;
//         }
//       `}</style>
