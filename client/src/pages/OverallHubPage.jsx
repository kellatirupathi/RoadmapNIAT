...
import React, { useState, useEffect, useCallback, useMemo } from 'react';
import { Navigate } from 'react-router-dom';
import { Card, Table, Spinner, Alert, Modal, Button, Badge, Form, InputGroup, Dropdown } from 'react-bootstrap';
import useAuth from '../hooks/useAuth.js';
import studentsTrackerService from '../services/studentsTrackerService.js';
import { ratingCalculations, sheetConfig } from '../utils/studentsTrackerConfig.js';
import overallHubService from '../services/overallHubService.js';
...
const OverallHubPage = () => {
    ...
    // --- NEW: State for the new dropdown filters ---
    ...
    const getClosingStatusFromItem = (item) => {
        const scores = [item.aseRating, item.companyInteractionRating, item.assignmentRating, item.incrutierRating, item.companyClosingRating];
        const validScores = scores.filter(score => score > 0);
        if (validScores.length === 0) return 'Risk';
        const averagePercentage = (validScores.reduce((sum, score) => sum + score, 0) / (validScores.length * 20)) * 100;
        if (averagePercentage >= 87.5) return 'Can Close';
        if (averagePercentage >= 50) return 'Moderate';
        return 'Risk';
    };
    
    const renderClosingStatus = (item) => {
        const status = getClosingStatusFromItem(item);
        let variant = 'danger';
        if (status === 'Can Close') variant = 'success';
        else if (status === 'Moderate') variant = 'warning';
        return <Badge bg={variant} pill>{status}</Badge>;
    };
    ...
    // --- NEW: Calculate counts for dropdowns ---
    const statusCounts = useMemo(() => {
        const counts = {
            closingStatus: { All: aggregatedData.length, 'Can Close': 0, 'Moderate': 0, 'Risk': 0 },
            placementStatus: { All: aggregatedData.length, 'Hired': 0, 'Hold': 0, 'Reject': 0, 'Not Set': 0 }
        };

        aggregatedData.forEach(item => {
            // Closing Status Count
            const closingStatus = getClosingStatusFromItem(item);
            if (counts.closingStatus[closingStatus] !== undefined) {
                counts.closingStatus[closingStatus]++;
            }

            // Placement Status Count
            const placementStatus = hubStatuses.get(`${item.companyName}|${item.niatId}`) || 'Not Set';
            if (counts.placementStatus[placementStatus] !== undefined) {
                counts.placementStatus[placementStatus]++;
            }
        });
        
        return counts;
    }, [aggregatedData, hubStatuses]);
    ...
    const filteredAndSortedData = useMemo(() => {
        ...
        if (closingStatusFilter !== 'All') {
            dataToFilter = dataToFilter.filter(item => getClosingStatusFromItem(item) === closingStatusFilter);
        }

        if (placementStatusFilter !== 'All') {
            dataToFilter = dataToFilter.filter(item => {
                const status = hubStatuses.get(`${item.companyName}|${item.niatId}`);
                if (placementStatusFilter === 'Not Set') return !status;
                return status === placementStatusFilter;
            });
        }
        
        return dataToFilter.sort((a,b) => b.total - a.total);
    }, [aggregatedData, searchTerm, hubStatuses, closingStatusFilter, placementStatusFilter]);
    ...
    return (
        <div>
            <style>{`
                .form-select.status-hired {
                    background-color: #d1e7dd !important;
                    color: #0f5132 !important;
                    border-color: #b1dfc7 !important;
                    font-weight: 500;
                }
                .form-select.status-hold {
                    background-color: #fff3cd !important;
                    color: #664d03 !important;
                    border-color: #ffecb5 !important;
                    font-weight: 500;
                }
                .form-select.status-reject {
             ...
             <Card className="shadow-sm">
                <Card.Header as="h5" className="d-flex justify-content-between align-items-center flex-wrap gap-2">
                    ...
                    <div className="d-flex align-items-center gap-2">
                        ...
                        <Dropdown>
                            <Dropdown.Toggle variant="outline-secondary" size="sm" id="closing-status-filter">
                                Closing Status: {closingStatusFilter}
                            </Dropdown.Toggle>
                            <Dropdown.Menu>
                                {Object.entries(statusCounts.closingStatus).map(([status, count]) => (
                                    <Dropdown.Item key={status} onClick={() => setClosingStatusFilter(status)} className="dropdown-item-flex">
                                        <span>{status}</span> <Badge pill bg="secondary">{count}</Badge>
                                    </Dropdown.Item>
                                ))}
                            </Dropdown.Menu>
                        </Dropdown>
                        ...
                        <Dropdown>
                            <Dropdown.Toggle variant="outline-secondary" size="sm" id="placement-status-filter">
                                Placement Status: {placementStatusFilter}
                            </Dropdown.Toggle>
                            <Dropdown.Menu>
                                {Object.entries(statusCounts.placementStatus).map(([status, count]) => (
                                    <Dropdown.Item key={status} onClick={() => setPlacementStatusFilter(status)} className="dropdown-item-flex">
                                        <span>{status}</span> <Badge pill bg="secondary">{count}</Badge>
                                    </Dropdown.Item>
                                ))}
                            </Dropdown.Menu>
                        ...
                        <Button variant="outline-danger" size="sm" onClick={() => { setSearchTerm(''); setClosingStatusFilter('All'); setPlacementStatusFilter('All'); }}>Clear Filters</Button>
                    </div>
                </Card.Header>
                            ...
                            <Table striped bordered hover size="sm">
                                <thead className="table-light">
                                    <tr>
                                        ...
                                        <th className="text-center">Overall (/100)</th>
                                        <th className="text-center">Closing Status</th>
                                        <th className="text-center" style={{width: '130px'}}>Overall Status</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {filteredAndSortedData.map((item, index) => {
                                        ...
                                        <tr key={rowKey}>
                                            ...
                                            <td className={`text-center fw-medium ${getScoreColor(item.companyClosingRating, 20)}`} onClick={() => handleScoreClick(item, 'companyClosings')} style={{ cursor: 'pointer' }}>{item.companyClosingRating}</td>
                                            <td className={`text-center fw-bolder ${getScoreColor(item.total, 100)}`}>{item.total}</td>
                                            <td className="text-center">{renderClosingStatus(item)}</td>
                                            <td className="text-center">
                                                {actionLoading[rowKey] ? <Spinner size="sm" /> : (
                                                    canEdit ? (
