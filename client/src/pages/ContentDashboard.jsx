    ...
    <div className="content-dashboard">
      ...
      {loading ? (
        ...
        <>
          <Card className="border-0 shadow-sm mb-4">
            <Card.Header className="bg-white py-3 d-flex justify-content-between align-items-center">
              <div className="d-flex align-items-center"><h5 className="mb-0 me-2">Manage Tech Stacks</h5><Badge pill bg="secondary">{techStacks.length}</Badge></div>
              {canCreate && (<Link to="/newtechstack" className="btn btn-sm btn-primary"><i className="fas fa-plus me-2"></i>Add New</Link>)}
            </Card.Header>
            <Card.Body className="p-0"><div className="p-3 border-bottom"><Row className="g-3"><Col><InputGroup><InputGroup.Text><i className="fas fa-search"></i></InputGroup.Text><Form.Control placeholder="Search..." value={searchTerm} onChange={handleSearchChange} />{searchTerm && (<Button variant="outline-secondary" onClick={() => setSearchTerm('')}><i className="fas fa-times"></i></Button>)}</InputGroup></Col></Row></div>
              <div className="table-responsive"><Table hover className="align-middle mb-0"><thead className="bg-light">
                <tr><th>Tech Stack</th><th>Description</th><th>Items</th><th>Progress</th><th>Last Updated</th><th className="text-center">Actions</th></tr>
              </thead><tbody>
                {filteredTechStacks.length > 0 ? (
                  filteredTechStacks.map((ts) => (
                    <tr key={ts._id}>
                      ...
                      <td><ProgressBar now={calculateProgress(ts)} variant={calculateProgress(ts) > 70 ? 'success' : 'secondary'} style={{ height: '8px' }} /><span className="text-muted small ms-2">{calculateProgress(ts)}%</span></td>
                      ...
                      <td><div className="d-flex justify-content-center gap-2">
                        ...
                        <Button variant="outline-success" size="sm" onClick={() => exportTechStackToCSV(ts)} title="Export to CSV"><i className="fas fa-file-csv"></i></Button>
                      </div></td>
                    </tr>
                  ))
                ) : (
