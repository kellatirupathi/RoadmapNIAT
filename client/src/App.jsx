...
import AdminDashboard from './pages/AdminDashboard';
import InternshipsTracker from './pages/InternshipsTracker';
import CriticalPointsPage from './pages/CriticalPoints'; // --- NEW IMPORT ---
...
const AppRoutes = () => {
  ...
  // Permission checks
  const canViewTechStacks = user.role === 'admin' || user.role === 'manager' || user.role === 'content' || (user.role === 'instructor' && (user.techStackPermission === 'view' || user.techStackPermission === 'edit'));
  ...
  const canAccessCriticalPoints = user.role === 'admin' || user.role === 'manager' || user.role === 'crm' || (user.role === 'instructor' && user.canAccessCriticalPoints);
        ...
        <main className="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100 p-4 md:p-6 relative">
          ...
          <div className={`transition-opacity duration-300 ${pageLoading ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
            <Routes>
              ...
              <Route path="/profile" element={
                <ProtectedRoute element={<ProfileSettings setPageLoading={setPageLoading} />} />
              } />

              <Route path="/internships-tracker" element={
                <ProtectedRoute element={<InternshipsTracker />} requiredRoles={['admin', 'manager']} />
              } />

              {/* --- START NEW ROUTE --- */}
              <Route path="/critical-points" element={
                 canAccessCriticalPoints ? <ProtectedRoute element={<CriticalPointsPage />} /> : <Navigate to="/not-authorized" replace />
              } />
              {/* --- END NEW ROUTE --- */}
              
              <Route path="/users" element={
                <ProtectedRoute element={<UserManagement setPageLoading={setPageLoading} />} requiredRoles={['admin']} />
              } />
              
              {canViewTechStacks && (
                <Route path="/alltechstacks" element={
                  <ProtectedRoute element={<ContentDashboard setPageLoading={setPageLoading} />} />
                } />
              )}
              
              <Route path="/roadmaps" element={
                <ProtectedRoute element={<RoadmapsList setPageLoading={setPageLoading} />} requiredRoles={['admin', 'manager', 'instructor', 'crm', 'content']} />
              } />
              
              {canManageRoadmaps && (
                <Route path="/manage-roadmaps" element={
                  <ProtectedRoute element={<RoadmapsManagement setPageLoading={setPageLoading} />} />
                } />
              )}
              
              {canCreateTechStack && (
                <Route path="/newtechstack" element={
                  <ProtectedRoute element={<AddTechStackForm onTechStackAdded={() => {}} />} />
                } />
              )}
              
              <Route path="/timeline" element={
                <ProtectedRoute element={<TimelineView setPageLoading={setPageLoading} />} requiredRoles={['admin', 'instructor', 'manager']} />
              } />
