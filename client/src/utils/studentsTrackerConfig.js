...
export const sheetConfig = {
    ...
    companyClosings: {
        ...
        columns: [
            ...
            { header: 'Remarks', field: 'remarks' }, 
            { header: 'Overall Marks', field: 'overallMarks', readOnly: true },
            { header: 'History', field: 'editHistory', minWidth: '150px' } 
        ]
    },
};
...
// I will now provide the full, corrected version of the `calculateCompanyClosingScore` function.
export const calculateCompanyClosingScore = (allClosings) => {
    const studentScores = {};
    allClosings.forEach(c => {
        // --- START FIX ---
        // Create a unique key for each student at each company. Using NIAT ID is more reliable.
        const key = `${c.companyName}|${c.niatId}`;
        // --- END FIX ---
    ...
    return allClosings.map(row => {
        // --- START FIX ---
        // Use the same key generation logic as above, using the current `row` object.
        const key = `${row.companyName}|${row.niatId}`;
        const scores = studentScores[key] || [];
        
        // Use Math.max to find the highest score, which correctly handles negative numbers.
        // If there are no scores, default to 0.
        const maxScore = scores.length > 0 ? Math.max(...scores) : 0;
        
        // Assign the correct 'marks' for the current row's rating, and the calculated 'maxScore' for 'overallMarks'.
        return { 
            ...row, 
            marks: companyClosingRatingMap.get(row.rating) || 0, 
            overallMarks: `${maxScore} / 20` 
        };
        // --- END FIX ---
    });
};
