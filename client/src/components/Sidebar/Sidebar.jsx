...
//   const [isCollapsed, setIsCollapsed] = useState(false); // New state for mini-sidebar
//   const [isHovered, setIsHovered] = useState(false); // New state for hover expansion
//   const [justCollapsed, setJustCollapsed] = useState(false); // Prevent immediate hover after collapse
...
//     const internshipsTrackerLink = { to: "/internships-tracker", icon: "M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm-1 9h-2v4h2v-4zm-2-2h2V5h-2v4z", label: "Internships Tracker"};
...
//       )}
//       <div 
//         className={`fixed inset-y-0 left-0 z-30 flex flex-col bg-gray-900 text-gray-100 transform transition-all duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-auto lg:z-auto shadow-2xl ${
...
//                 className="hidden lg:block text-gray-400 hover:text-white focus:outline-none p-2 rounded-md transition-colors duration-200"
...
//                     className={`flex items-center px-4 py-3 text-sm font-semibold rounded-lg transition-all duration-200 ease-in-out bg-primary-500 text-white hover:bg-primary-600 shadow-lg hover:shadow-primary-500/40 transform hover:-translate-y-0.5 ${
//                       isCollapsed && !isHovered ? 'justify-center px-2' : 'justify-center'
//                     }`}
//                     title={isCollapsed && !isHovered ? link.label : ''}
//                   >
//                     <svg className={`h-5 w-5 flex-shrink-0 ${(isExpanded || window.innerWidth < 1024) ? 'mr-2' : ''}`} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
//                 onClick={(e) => handleNavigation(link.to, e)}
//                 className={({ isActive }) => 
//                   `group flex items-center py-2.5 text-sm font-medium rounded-md transition-all duration-200 ease-in-out relative ${
//                     (isActive || (link.to !== "/" && activeSection.startsWith(link.to))) 
//                       ? 'bg-primary-500 text-white shadow-md' 
//                       : 'text-gray-300 hover:bg-gray-800 hover:text-white'
//                   } ${isCollapsed && !isHovered ? 'px-2 justify-center' : 'px-4'}`
//                 }
//                 end={link.to === "/"}
//                 title={isCollapsed && !isHovered ? link.label : ''}
//               >
//                 <svg className={`h-5 w-5 flex-shrink-0 ${(isExpanded || window.innerWidth < 1024) ? 'mr-3' : ''}`} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
//                   <path d={link.icon} fill="currentColor"/>
//                 </svg>
...
//               className={`group flex items-center w-full py-2.5 text-sm font-medium rounded-md transition-all duration-200 ease-in-out text-gray-400 hover:bg-red-600 hover:text-white ${
//                 isCollapsed && !isHovered ? 'px-2 justify-center' : 'px-4'
//               }`}
...
//                 <div className="w-10 h-10 rounded-full bg-primary-500 flex items-center justify-center text-white font-semibold text-lg shadow-inner flex-shrink-0">
...
//             <div className="px-2 py-3 border-t border-gray-800 bg-gray-800/50 flex justify-center">
//               <div 
//                 className="w-10 h-10 rounded-full bg-primary-500 flex items-center justify-center text-white font-semibold text-lg shadow-inner cursor-pointer"
...
const Sidebar = ({ isOpen, toggleSidebar, setPageLoading }) => {
  ...
  const getNavLinks = () => {
    ...
    const canManageRoadmaps = user.role === 'admin' || user.role === 'manager' || (user.role === 'instructor' && user.canManageRoadmaps);
    const hasTechStackViewAccess = user.role === 'admin' || user.role === 'content' || user.role === 'manager' || (user.role === 'instructor' && (user.techStackPermission === 'view' || user.techStackPermission === 'edit'));
    const canCreateTechStack = user.role === 'admin' || user.role === 'content' || (user.role === 'instructor' && user.techStackPermission === 'edit');
    const canAccessCriticalPoints = user.role === 'admin' || user.role === 'manager' || (user.role === 'instructor' && user.canAccessCriticalPoints);

    let navLinks = [...commonLinks];

    const instructorAndManagerLinks = [
        { to: "/internships-tracker", icon: "M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm-1 9h-2v4h2v-4zm-2-2h2V5h-2v4z", label: "Internships Tracker" },
    ];

    const timelineAndRoadmapLinks = [
        { to: "/timeline", icon: "M9 11H7v2h2v-2zm4 0h-2v2h2v-2zm4 0h-2v2h2v-2zm2-7h-1V2h-2v2H8V2H6v2H5c-1.11 0-1.99.9-1.99 2L3 20c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 16H5V9h14v11z", label: "Timeline View" },
    ];
    
    // Using a new Set to avoid duplicate entries from different role permissions
    const linkSet = new Set(navLinks.map(l => l.to));

    // Admin
    if (user.role === 'admin') {
      [...instructorAndManagerLinks, 
      { to: "/critical-points", icon: "M20.285 2l-2.857 2.857-1.428-1.428-1.428 1.428-2.143-2.143L9.571 5.571l-1.428-1.428L5.286 7l-2.143 2.143 2.857 2.857-1.428 1.428 1.428 1.428 2.143-2.143L11.571 16l1.428 1.428-2.857 2.857 2.143 2.143L16 19.571l1.428 1.428 2.857-2.857L23.143 16l-2.858-2.857zM11.857 9.143L8 5.286l-2.143 2.143 3.857 3.857-2.857 2.857 1.428 1.428 3.857-3.857L16 16l-1.428-1.428L10.714 10.714l1.143-1.571z", label: "Critical Points" },
       ...timelineAndRoadmapLinks,
       { to: "/alltechstacks", icon: "M4 7h16v2H4V7zm0 4h16v2H4v-2zm0 4h16v2H4v-2z", label: "Tech Stacks" },
       { to: "/manage-roadmaps", icon: "M13 21v2.5l-3-2-3 2V21h-.5A3.5 3.5 0 0 1 3 17.5V5a3 3 0 0 1 3-3h14a1 1 0 0 1 1 1v17a1 1 0 0 1-1 1h-7zm-6-2v-2h6v2h6v-3H6.5a1.5 1.5 0 0 0 0 3H7zM7 5v2h2V5H7zm0 3v2h2V8H7zm0 3v2h2v-2H7z", label: "Roadmaps" },
       { to: "/roadmaps", icon: "M20.59 2.59C20.21 2.21 19.7 2 19.17 2H4.83C3.17 2 1.83 3.34 1.83 5L1.5 17.01c0 1.66 1.34 3 3 3h15c1.66 0 3-1.34 3-3V7.83c0-.53-.21-1.04-.59-1.41L14.17 2.59zM14 4.5V8h4.5L14 4.5zM5.5 17V5h7.5v4.5H17v7.5H5.5z", label: "Github Files" },
       { to: "/users", icon: "M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z", label: "User Management" },
       { to: "/newtechstack", type: 'button', icon: "M11 11V5h2v6h6v2h-6v6h-2v-6H5v-2h6z", label: "New Tech Stack" }
      ].forEach(link => { if(!linkSet.has(link.to)) navLinks.push(link) });
    }
    // Content Team
    ...
    // Manager
       ...
       managerLinks.forEach(link => { if(!linkSet.has(link.to)) navLinks.push(link) });
       if(hasTechStackViewAccess && !linkSet.has('/alltechstacks')) navLinks.push({ to: "/alltechstacks", icon: "M4 7h16v2H4V7zm0 4h16v2H4v-2zm0 4h16v2H4v-2z", label: "Tech Stacks" });
       if(canManageRoadmaps && !linkSet.has('/manage-roadmaps')) navLinks.push({ to: "/manage-roadmaps", icon: "M13 21v2.5l-3-2-3 2V21h-.5A3.5 3.5 0 0 1 3 17.5V5a3 3 0 0 1 3-3h14a1 1 0 0 1 1 1v17a1 1 0 0 1-1 1h-7zm-6-2v-2h6v2h6v-3H6.5a1.5 1.5 0 0 0 0 3H7zM7 5v2h2V5H7zm0 3v2h2V8H7zm0 3v2h2v-2H7z", label: "Roadmaps" });
    }
    // Instructor
    else if (user.role === 'instructor') {
      const instructorLinks = [];
      if (canAccessCriticalPoints) instructorLinks.push({ to: "/critical-points", icon: "M20.285 2l-2.857 2.857-1.428-1.428-1.428 1.428-2.143-2.143L9.571 5.571l-1.428-1.428L5.286 7l-2.143 2.143 2.857 2.857-1.428 1.428 1.428 1.428 2.143-2.143L11.571 16l1.428 1.428-2.857 2.857 2.143 2.143L16 19.571l1.428 1.428 2.857-2.857L23.143 16l-2.858-2.857zM11.857 9.143L8 5.286l-2.143 2.143 3.857 3.857-2.857 2.857 1.428 1.428 3.857-3.857L16 16l-1.428-1.428L10.714 10.714l1.143-1.571z", label: "Critical Points" });
      instructorLinks.push(...timelineAndRoadmapLinks);
      
      instructorLinks.forEach(link => { if(!linkSet.has(link.to)) navLinks.push(link) });
      if(canManageRoadmaps && !linkSet.has('/manage-roadmaps')) navLinks.push({ to: "/manage-roadmaps", icon: "M13 21v2.5l-3-2-3 2V21h-.5A3.5 3.5 0 0 1 3 17.5V5a3 3 0 0 1 3-3h14a1 1 0 0 1 1 1v17a1 1 0 0 1-1 1h-7zm-6-2v-2h6v2h6v-3H6.5a1.5 1.5 0 0 0 0 3H7zM7 5v2h2V5H7zm0 3v2h2V8H7zm0 3v2h2v-2H7z", label: "Roadmaps" });
      if(hasTechStackViewAccess && !linkSet.has('/alltechstacks')) navLinks.push({ to: "/alltechstacks", icon: "M4 7h16v2H4V7zm0 4h16v2H4v-2zm0 4h16v2H4v-2z", label: "Tech Stacks" });
      if(canCreateTechStack && !linkSet.has('/newtechstack')) navLinks.push({ to: "/newtechstack", type: 'button', icon: "M11 11V5h2v6h6v2h-6v6h-2v-6H5v-2h6z", label: "New Tech Stack" });
    }
    ...
    // Sort navLinks by a predefined order
    const orderedPaths = [ "/", "/internships-tracker", "/critical-points", "/timeline", "/alltechstacks", "/manage-roadmaps", "/roadmaps", "/users", "/newtechstack"];
    
    // Using a map for quick lookups
    const linkMap = new Map(navLinks.map(link => [link.to, link]));
    
    const sortedLinks = orderedPaths.filter(path => linkMap.has(path)).map(path => linkMap.get(path));
