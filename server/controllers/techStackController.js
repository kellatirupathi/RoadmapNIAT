...
export const createTechStack = async (req, res) => {
  try {
    const canCreate = req.user.role === 'admin' || req.user.role === 'content' || (req.user.role === 'instructor' && req.user.techStackPermission === 'edit');
    if (!canCreate) {
      return res.status(403).json({ success: false, error: 'You do not have permission to create tech stacks.' });
    }
    ...
    console.error('Error in createTechStack:', error);
    if (error.code === 11000) {
      return res.status(400).json({ success: false, error: 'Tech stack with this name already exists' });
    }
    res.status(500).json({ success: false, error: 'Server Error' });
  }
};
...
export const updateTechStack = async (req, res) => {
  try {
    const canEdit = req.user.role === 'admin' || req.user.role === 'content' || (req.user.role === 'instructor' && req.user.techStackPermission === 'edit');
    if (!canEdit) {
      return res.status(403).json({ success: false, error: 'You do not have permission to edit tech stacks.' });
    }

    const techStack = await TechStack.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
    if (!techStack) {
      return res.status(404).json({ success: false, error: 'Tech stack not found' });
    }
    res.status(200).json({ success: true, data: techStack });
...
export const deleteTechStack = async (req, res) => {
  try {
    const canDelete = req.user.role === 'admin' || req.user.role === 'content' || (req.user.role === 'instructor' && req.user.techStackPermission === 'edit');
    if (!canDelete) {
      return res.status(403).json({ success: false, error: 'You do not have permission to delete tech stacks.' });
    }
...
export const deleteAllTechStacks = async (req, res) => {
  try {
    await TechStack.deleteMany({});
    res.status(200).json({ success: true, message: 'All tech stacks have been deleted successfully', data: {} });
  } catch (error) {
    console.error('Error in deleteAllTechStacks:', error);
    res.status(500).json({ success: false, error: 'Failed to delete all tech stacks' });
  }
};
...
export const addRoadmapItem = async (req, res) => {
  try {
    const canAdd = req.user.role === 'admin' || req.user.role === 'content' || (req.user.role === 'instructor' && req.user.techStackPermission === 'edit');
    if (!canAdd) {
      return res.status(403).json({ success: false, error: 'You do not have permission to add items.' });
    }
...
const internalUploadToGithub = async ({ filename, content, message }) => {
  ...
  const body = { message: message || `Update ${filename} via system`, content: Buffer.from(content).toString('base64'), branch: GITHUB_BRANCH, ...(sha && { sha }) };
  const uploadRes = await fetch(repoPath, { method: 'PUT', headers, body: JSON.stringify(body) });
  ...
  return { github_api_response: responseData, published_url: publishedPageUrl };
};

const syncGitHubRoadmapsForTechStack = async (updatedTechStackId) => {
  const updatedTechStack = await TechStack.findById(updatedTechStackId);
  if (!updatedTechStack) return;

  const affectedRoadmaps = await Roadmap.find({ $or: [{ techStacks: updatedTechStack.name, isConsolidated: false }, { 'roles.techStacks': updatedTechStack._id, isConsolidated: true }]}).populate({ path: 'roles.techStacks', select: 'name' });
  if (affectedRoadmaps.length === 0) return;

  for (const roadmap of affectedRoadmaps) {
    try {
      let techStackNamesForThisRoadmap = new Set();
      if (roadmap.isConsolidated) {
        roadmap.roles.forEach(role => role.techStacks.forEach(ts => { if(ts && ts.name) techStackNamesForThisRoadmap.add(ts.name); }));
      } else {
        roadmap.techStacks.forEach(name => techStackNamesForThisRoadmap.add(name));
      }
      const uniqueTechStackNamesArray = Array.from(techStackNamesForThisRoadmap);
      const fullTechStacksDataForRoadmap = await TechStack.find({ name: { $in: uniqueTechStackNamesArray } });
      ...
      if (roadmap.isConsolidated) {
        rolesForHtmlGeneration = roadmap.roles.map(roleDetail => ({
          title: roleDetail.title,
          techStacks: fullTechStacksDataForRoadmap.filter(fullTS => roleDetail.techStacks.some(tsInRole => tsInRole.name === fullTS.name))
        }));
      } else {
        rolesForHtmlGeneration = [{ title: roadmap.role, techStacks: fullTechStacksDataForRoadmap }];
      }
      ...
      console.error(`[GitHub Sync Service] Failed to update roadmap "${roadmap.filename}" on GitHub:`, syncError.message);
    }
  }
};
...
export const updateRoadmapItem = async (req, res) => {
  try {
    const isFullEditPermitted = req.user.role === 'admin' || req.user.role === 'content' || (req.user.role === 'instructor' && req.user.techStackPermission === 'edit');
    const isStatusUpdateOnly = Object.keys(req.body).length === 1 && req.body.hasOwnProperty('completionStatus');

    if (!isFullEditPermitted && !(req.user.role === 'instructor' && isStatusUpdateOnly)) {
        return res.status(403).json({ success: false, error: 'You do not have permission to perform this update.' });
    }
    ...
    const itemIndex = techStack.roadmapItems.findIndex(item => item._id.toString() === req.params.itemId);
    if (itemIndex === -1) {
      return res.status(404).json({ success: false, error: 'Roadmap item not found' });
    }
    
    techStack.roadmapItems[itemIndex] = { ...techStack.roadmapItems[itemIndex].toObject(), ...req.body };
    await techStack.save();
    const updatedParentTechStack = await TechStack.findById(req.params.id);
    ...
    syncGitHubRoadmapsForTechStack(req.params.id).catch(syncError => {
      console.error(`[Background GitHub Sync] Error after updating item ${req.params.itemId}:`, syncError.message);
    });
  } catch (error) {
    console.error('Error in updateRoadmapItem:', error);
    res.status(500).json({ success: false, error: 'Server Error' });
...
export const deleteRoadmapItem = async (req, res) => {
  try {
    const canDelete = req.user.role === 'admin' || req.user.role === 'content' || (req.user.role === 'instructor' && req.user.techStackPermission === 'edit');
    if (!canDelete) {
        return res.status(403).json({ success: false, error: 'You do not have permission to delete items.' });
    }
    ...
    techStack.roadmapItems = techStack.roadmapItems.filter(item => item._id.toString() !== req.params.itemId);
    await techStack.save();
    
    res.status(200).json({ success: true, data: techStack });

    syncGitHubRoadmapsForTechStack(req.params.id).catch(syncError => {
      console.error(`[Background GitHub Sync] Error after deleting item ${req.params.itemId}:`, syncError.message);
    });
  } catch (error) {
    console.error('Error in deleteRoadmapItem:', error);
    res.status(500).json({ success: false, error: 'Server Error' });
...
export const updateRoadmapItemSchedule = async (req, res) => {
  try {
    ...
    if (scheduledDate !== null && scheduledDate !== undefined && scheduledDate !== '' && !/^\d{4}-\d{2}-\d{2}$/.test(scheduledDate)) {
      return res.status(400).json({ success: false, error: 'Invalid date format. Please use YYYY-MM-DD or null.' });
    }

    const instructor = await User.findById(req.user.id).select('assignedTechStacks');
    if (req.user.role !== 'admin' && (!instructor || !instructor.assignedTechStacks.map(id => id.toString()).includes(techStackId))) {
      return res.status(403).json({ success: false, error: 'Not authorized to schedule items for this tech stack.' });
    }
